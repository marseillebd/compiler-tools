...
runRecog action input = snd <$> unR (action input) st0
  ...
  st0 = St
    { context = Nothing
    , rest = Done $ spanFromPos input.span.start
    }
...
pop = R $ \st -> case st.rest of
  ...
  More (x:|[]) -> pure (st{rest = Done $ spanFromPos x.span.end}, Right x)
...
parse (p, trees) action = R $ \st -> do
  let inner = maybe (Done $ spanFromPos p) More $ NE.nonEmpty (toList trees)
